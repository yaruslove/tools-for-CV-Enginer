...
def rename_and_copy_files(src_dir, dst_dir):
    ...
    for filename in tqdm(json_files, desc="Processing files"):
        ...
        # Extract the base name by removing the image extension before .json
        # Pattern: match anything that ends with an image extension followed by .json
        pattern = r'(.+?)(\.(jpg|jpeg|png|bmp|gif))\.json$'
        match = re.match(pattern, filename, re.IGNORECASE)
        
        if match:
            # Get the base name (without image extension)
